...
const MapContainer = ({ match }) => {
  ...
  function sortRecentReviewByAsc() {
    setIsshown(true);
    const orderBy = _.orderBy(reviewData.data, ['createdAt'], ['desc']);
    setSortt(orderBy);
  }
        ...
        <div className="Map">
            ...
            <div id="Food-list">
              <div className="Food-head">
                <SearchBar
                  ...
                  style={{ width: '100 %' }}
                />
                <span id="itemState" onClick={fold}>
                  접기
                </span>
              </div>
          ...
          <div className="Map-bottom" style={{ padding: '10px' }}>
            <div className="Rank-list">
              <h2>음식점 랭킹</h2>
              <Button size="small" onClick={() => sortScoreByAsc()}>
                별점 높은 순
              </Button>{' '}
              &nbsp;&nbsp;&nbsp;
              <Button size="small" onClick={() => sortReviewByAsc()}>
                리뷰 많은 순
              </Button>{' '}
              <h6>버튼을 클릭해주세요!</h6>
              {ranker?.map((d, index) => {
            ...
            <div className="Last-review" style={{ left: '40%' }}>
              <h2>최신순 리뷰</h2>
              <Button size="small" onClick={() => sortRecentReviewByAsc()}>
                버튼을 클릭해주세요!
              </Button>
              {sortt?.map((d, index) => {
                if (index < 2) {
